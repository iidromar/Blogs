...
if(strtolower($username) != strtolower($row['username'])){
    header('location:index.php?error=You can only edit your own blogs!');
}
...
<div class="section">
    <div class="container">
        <div class="section-row">
            ...
            <form class="post-reply" method="post" action="processEdit.php?Bid=<?php echo $Bid ?>" enctype="multipart/form-data">
                <div class="row">
                    <div class="col-md-4">
                        <div class="form-group">
                            <label for="title" class="form-label">Blog's Title:</label>
                            <input type="text" name="title" id="title" class="form-control"
                                   placeholder="Ex. Top 5 JavaScript courses" maxlength="50" value="<?php echo $title;?>" required>
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="form-group">
                            <label for="image" class="form-label">Images (by uploading new image old image is removed):</label>
                            <input class="form-control" type="file" id="image" name="userImage">
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="form-group">
                            <label for="video" class="form-label">Videos (by uploading new video old video is removed):</label>
                            <input class="form-control" type="file" id="video" name="video">
                        </div>
                    </div>
                    <div class="col-md-2">
                        <div class="form-group">
                            <label for="category" class="form-label">Category: <?php echo $category ?></label>
                            <select class="form-select form-control" aria-label="Default select example" id="category"
                                    name="category" required>
                                <option value="food" <?php if('food' == $category) echo  'selected';?>>Food</option>
                                <option value="technology"<?php if('technology' == $category) echo  'selected';?>>Technology</option>
                                <option value="health"<?php if('health' == $category) echo  'selected';?>>Health</option>
                                <option value="travel"<?php if('travel' == $category) echo  'selected';?>>Travel</option>
                            </select>
                        </div>
                    </div>
                </div>
                <div class="row">
                    <div class="col-md-12">
                        <div class="form-group">
                            ...
                            <textarea class="form-control" id="text" name="text" maxlength="2000" rows="12" required><?php echo $text;?></textarea>
                        </div>
                    </div>
                </div>
                <div class="row">
                    <span class="col-md-6"><div class="form-check">
                            <input class="form-check-input" type="checkbox" value="keepimages" id="keepimages" name="keepimages"<?php if($imgid == '') echo 'disabled'?>>
                            <label class="form-check-label" for="keepimages">
                                Keep same images
                            </label>
                        </div>
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" value="keepvideos" id="keepvideos" name="keepvideos" <?php if($vidid == '') echo 'disabled' ?>>
                            <label class="form-check-label" for="keepvideos">
                                Keep same videos
                            </label>
                        </div>
                    </span>
