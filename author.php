<?php
	...
	if(!isset($_GET['author'])){
		...
		if($_GET['author'] != $username){
			header('Location:author2.php?author='.$_GET['author']);
		}
	}
	include 'DB_CONFIG.PHP';
	$conf = mysqli_connect(DBHOST,DBUSER,DBPWD,DBNAME);
	$isEmpty = false;
	$queryi = "select identity from blogger where username = '".$username."'";
	$resulti = mysqli_query($conf, $queryi);
	$identity = mysqli_fetch_assoc($resulti);
	if(strtolower($identity['identity']) == 'user'){
		header('Location:index.php?error=You are not a blogger');
	}
?>
<!DOCTYPE html>
<html lang="en">
                ...
                <div class="col-md-7">
                    <div class="form-group">
                        <form action="createBlog.php" method="post">
                            <button type="submit" class="primary-button">New Blog</button>
                        </form>
                    </div>
                </div>
            </div>
			...
			<div class="row">
                    ...
                    //for($i = 0; $i < mysqli_num_rows($result); $i++){
                        ...
                        if ($imgid > 0) {
                            ...
                            echo "<div class='post post-row'>
									...
									echo "<br><p class='alert-danger' style='text-align: center;'>You have no blogs!</p>";
									$isEmpty = true;
								}
                ?>
                    ...
                    <script type="text/javascript">
                        ...
                        {
                            if (confirm("Are you sure you want to delete this blog? Blog id = " + i + ""))
                                location.href='deleteBlog.php?Bid='+i+'&author=<?php echo $username ?>';
                        }
                    </script>
				...
				<div class="col-md-4" style="float: right;">
					<div class="aside-widget">
							<?php
							if(!$isEmpty){
							echo "<h3 style='text-align: center;'>Top Rated Blog</h3>";
						    ...
						    die("Cannot connect to database.".mysqli_connect_errno());
								...
								if(mysqli_num_rows($result) == 1){
									...
									echo "<br><p class='alert-danger' style='text-align: center;'>There are no rated blogs!</p>";
								}
							}
							else{
								echo "<br><p class='alert-danger' style='text-align: center'>You have no blogs!</p>";
					...
					<div class="aside-widget">
						<br>
							<?php
							if(!$isEmpty){
								echo "<h3 style='text-align: center;'>Most Commented Blog</h3>";
								...
								if(mysqli_num_rows($result) > 0){
									while($highestRow = mysqli_fetch_assoc($result)){
										...
										echo "<div style='height: 100%; background-color: #EEEEEE; overflow: auto; padding-bottom: 10px;'><br>
											<div class='post-body' style='float: left; width: 50%; border-right: 1px solid #DDDDDD; margin-left:25px'>
												<div class='post-category'>
													<a href='category.php'>".$highestBlogRow['category']."</a>
												</div>
												<h4 class='post-title'><a href='blog-post.php'>".$highestBlogRow["title"]."</a></h4>
												<ul class='post-meta'>
													<li><a href='author.php?author=".$username."'>" . $username . "</a></li>
